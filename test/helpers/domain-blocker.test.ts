...
describe('Domain Blocker', () => {
  ...
  describe('validateTestRequest', () => {
    it('allows non-blocked domains', () => {
      expect(() => validateTestRequest('http://localhost:61969/live.php')).not.toThrow();
      expect(() => validateTestRequest('https://example.com/api')).not.toThrow();
    });

    it('allows image requests to blocked domains', () => {
      expect(() => validateTestRequest('https://oracle.browse.wf/logo.png')).not.toThrow();
      expect(() => validateTestRequest('https://browse.wf/image.jpg')).not.toThrow();
    });

    it('throws for non-image requests to oracle.browse.wf', () => {
      expect(() => validateTestRequest('https://oracle.browse.wf/min'))
        .toThrow(/TEST SAFEGUARD.*production domain/);
      expect(() => validateTestRequest('https://oracle.browse.wf/worldState.json'))
        .toThrow(/TEST SAFEGUARD.*production domain/);
    });

    it('throws for non-image requests to browse.wf', () => {
      expect(() => validateTestRequest('https://browse.wf/arbys.txt'))
        .toThrow(/TEST SAFEGUARD.*production domain/);
      expect(() => validateTestRequest('https://www.browse.wf/api'))
        .toThrow(/TEST SAFEGUARD.*production domain/);
    });
