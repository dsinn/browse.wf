...
const alertsScheduledExpiries = new Set();
let renderedAlertOids;
let renderedGoals = "";
...
ExportImages_promise.then(res => { window.ExportImages = res; });
...
function updateBountyCycleLocalised() {
    ...
    for (const syndicateTag of ["HexSyndicate", "EntratiLabSyndicate", "ZarimanSyndicate"]) {
        ...
        for (let i = 0; i != window.bountyCycle.bounties[syndicateTag].length; ++i) {
            ...
            rows[i].dataset.missionType = node.missionType;
            ...
            if (window.bountyCycle.bounties[syndicateTag][i].ally) {
                window.renderAllyIcon(allyNames[window.bountyCycle.bounties[syndicateTag][i].ally], rows[i].querySelector(".ally"));
            }
        ...
        window.applyBountyFilters(syndicateTag, rows);
    }
}
...
async function updateArbyLocalised() {
    ...
    addTooltip(arbyWhereElem, window.formatTileset(window.getTileset(window.arby_node)));
...
function updateIncursions() {
    ...
    void window.updateIncursionsLocalised();
...
function updateWorldStateLocalised() {
    void window.updateNewsTicker();
    updateAlerts();
    updateGoals();
    void window.updateFissures();
    updateInvasions();
}
function fetchWorldState() {
    return window.WarframeApiFrontProxyClient.fetchWorldState().then((worldState) => {
        window.worldState = worldState;
        updateWorldStateLocalised();
    });
}
function initWorldStateCards() {
    updateDayNightCycle();
    updateSorties();
    updateArchonHunt();
    updateDarvosDeal();
...
async function updateSorties() {
    ...
    if (!sortie) {
        setTimeout(updateSorties, STALE_DATA_RETRY_MS);
        return;
    }
    ...
    for (const variant of sortie.Variants) {
        ...
        window.appendSortieLocation(td, ExportRegions[variant.node], variant.tileset);
        tr.appendChild(td);
...
async function updateDarvosDeal() {
    ...
    if (!window.dailyDeal) {
        setTimeout(updateDarvosDeal, STALE_DATA_RETRY_MS);
        return;
    }
...
async function updateAlerts(forceRender = false) {
    ...
    const currentOids = new Set(window.worldState.Alerts.map((a) => a._id.$oid));
    ...
    if (window.worldState.Alerts.length != 0) {
        ...
        const sortedAlerts = [...window.worldState.Alerts].sort((a, b) => {
        ...
        const activeAlertExpiries = new Set(sortedAlerts
            .filter((a) => Date.now() >= a.Activation.$date.$numberLong && Date.now() < a.Expiry.$date.$numberLong)
            .map((a) => Number.parseInt(a.Expiry.$date.$numberLong, 10)));
        for (const expiry of activeAlertExpiries) {
            if (alertsScheduledExpiries.has(expiry))
                continue;
    ...
    if (!sameOids && prevOids) {
        const newAlerts = window.worldState.Alerts.filter((a) => !prevOids.has(a._id.$oid));
...
async function updateGoals(forceRender = false) {
    ...
    if (!forceRender && content === renderedGoals)
        return;
...
function toggleOidCompletion(oid) {
    ...
    window.triggerCloudSyncWithDebounce();
}
function createCompletionToggle(oid) {
    ...
    a.onclick = function () {
        ...
        window.setCompletionToggle(a, newCompletedState);
        window.applyCheckboxLinking(a, newCompletedState);
    };
    return a;
}
const fissureTiers = {
...
initializeCardFilters('news', () => void window.updateNewsTicker(true));
initializeCardFilters('incursions', () => void window.updateIncursionsLocalised());
initializeCardFilters('fissures', () => void window.updateFissures(true));
initializeCardFilters('sp-fissures', () => void window.updateFissures(true));
initializeCardFilters('rj-fissures', () => void window.updateFissures(true));
initializeCardFilters('weekly-missions', () => window.filterWeeklyMissions());
...
window.createCompletionToggle = createCompletionToggle;
window.dicts_promise = dicts_promise;
window.ExportRegions_promise = ExportRegions_promise;
window.fissureTiers = fissureTiers;
window.getItemNamePromise = getItemNamePromise;
window.isOidMarkedAsCompleted = isOidMarkedAsCompleted;
window.refreshCollapseStatus = refreshCollapseStatus;
window.refreshNotifStatus = refreshNotifStatus;
window.setDatum = setDatum;
window.toggleOidCompletion = toggleOidCompletion;
window.toTitleCase = toTitleCase;
window.updateBountyCycleLocalised = updateBountyCycleLocalised;
window.initLiveSync();
