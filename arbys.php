...
<body data-bs-theme="dark">
	...
	<div class="p-3">
		<p>
			...
			<button id="btn-save-settings" class="btn btn-sm btn-primary ms-3">Save settings</button>
			<button id="btn-load-settings" class="btn btn-sm btn-primary ms-2" disabled>Restore settings</button>
		</p>
		<div class="row">
			...
			<div class="col-xl-6">
				<table class="table">
					...
					<tbody>
						...
						foreach ($tilesets as $id):
							// Format tileset: remove "Tileset" and convert PascalCase to Title Case
							$label = preg_replace('/Tileset/', '', $id);
							$label = preg_replace('/(?<=[a-z])(?=[A-Z])/', ' ', $label);
						?>
						<tr id="next-<?= $id ?>"><th><input id="filter-<?= $id ?>" type="checkbox" class="form-check-input" checked /> <label for="filter-<?= $id ?>"><?= $label ?></label></th><td></td><td></td></tr>
						<?php endforeach; ?>
