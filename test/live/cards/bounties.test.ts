...
describe('Bounties Card', () => {
  ...
  test('renders Zariman bounties from bounty-cycle', () => {
    ...
    zarimanBounties.forEach((bounty: any) => {
      expect(bounty).toHaveProperty('node');
      expect(bounty).toHaveProperty('challenge');
      expect(bounty.node).toMatch(/^SolNode\d+$/);
      expect(bounty.challenge).toMatch(/^\/Lotus\/Types\/Challenges\//);
    });
  });
  ...
  test('renders Hex bounties with allies', () => {
    ...
    expect(bountyWithAlly.ally).toMatch(/^\/Lotus\/Types\/Gameplay/);
  });
});
