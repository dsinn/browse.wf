...
function getTeshinOfferSchedule(): {epoch: number; week: number; teshinOffer: string[]} {
...
function formatWeekStart(epoch: number, week: number): string {
	const weekStartMs = epoch + (week * 604_800_000);
	return new Date(weekStartMs).toLocaleDateString('en', {month: 'short', day: 'numeric'});
}

export function renderTeshinForecastHeader(theadRow: HTMLElement): void {
	theadRow.innerHTML = '';
...
export function renderTeshinForecastTable(tbody: HTMLElement): void {
