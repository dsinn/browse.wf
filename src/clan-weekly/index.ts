...
export async function renderClanWeeklyPane(entry: any): Promise<HTMLDivElement> {
	...
	const regionName = resolveBonusRegion(entry.BonusRegion as string, dict);
...
export async function updateClanWeekly(): Promise<void> {
	const entries: any[] = (window as any).worldState?.WeeklyVaultBonusRewards ?? [];
