...
export type CyclePhase = 'night' | 'day';
export type HourFormat = 'mil' | '24' | '12';
 ...
 */
function getCycleExpiryAtOrAfter(from: number): number {
 ...
 */
export function getCycleWindows(phase: CyclePhase, from: number, spanMs: number): CycleWindow[] {
	...
	while (cycleExpiry < until + PERIOD_MS) {
		const window: CycleWindow = phase === 'night'
...
export function formatWindow(window: CycleWindow, timeZone: 'local' | 'utc', hourFormat: HourFormat = '24'): string {
	return `${formatTime(window.start, timeZone, hourFormat)}–${formatTime(window.end, timeZone, hourFormat)}`;
}

function formatTime(timestamp: number, timeZone: 'local' | 'utc', hourFormat: HourFormat): string {
...
export function getDayBucket(timestamp: number, timeZone: 'local' | 'utc'): number {
	const date = new Date(timestamp);
	if (timeZone === 'utc') {
 ...
 * time in local mode.
 */
export function addCalendarDays(dayBucket: number, dayOffset: number, timeZone: 'local' | 'utc'): number {
	if (timeZone === 'utc') {
...
export function formatDayHeading(dayBucket: number, timeZone: 'local' | 'utc'): string {
...
export function groupWindowsByDay(
	windows: CycleWindow[],
	startDayBucket: number,
	dayCount: number,
	timeZone: 'local' | 'utc',
): Map<number, CycleWindow[]> {
	const days = new Map<number, CycleWindow[]>();
...
export function isWindowPast(window: CycleWindow, now: number): boolean {
	return window.end < now;
}

/** True if `now` falls within the window (it's the currently in-progress cycle). */
export function isWindowActive(window: CycleWindow, now: number): boolean {
 ...
 */
export function getNextBoundary(now: number, timeZone: 'local' | 'utc'): number {
