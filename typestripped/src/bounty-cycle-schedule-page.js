import { getCycleWindows, formatWindow, getDayBucket, formatDayHeading, groupWindowsByDay, isWindowPast, isWindowActive, getNextBoundary, MAX_WINDOWS_PER_DAY, } from './bounty-cycle-schedule.js';
const DAY_COUNT = 7;
...
function getPhase() {
    const toggle = document.querySelector('#phase-toggle');
 ...
 */
function* dayColumns(headingsRow, grouped, now) {
...
function render() {
    ...
    const headingsRow = document.querySelector('#schedule-table-headings');
...
function setCelestialBody(element, active, animate) {
...
function setCelestialState(phase, animate) {
...
function updatePhaseLabels(phase) {
    document.querySelector('#phase-label-day').classList.toggle('is-active-phase', phase === 'day');
    document.querySelector('#phase-label-night').classList.toggle('is-active-phase', phase === 'night');
}
...
let scheduledMidTransitionRender;
...
document.querySelector('#phase-toggle').addEventListener('change', onPhaseChange);
document.querySelector('#select-tz').addEventListener('change', onTimeZoneChange);
document.querySelector('#select-hourfmt').addEventListener('change', render);
applyTheme(getPhase());
setCelestialState(getPhase(), false);
updatePhaseLabels(getPhase());
render();
scheduleNextRerender();
//# sourceMappingURL=bounty-cycle-schedule-page.js.map