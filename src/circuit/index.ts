...
function formatWeekDate(ms: number): string {
	return new Date(ms).toLocaleDateString('en', {month: 'short', day: 'numeric'});
}

function choicesToDisplay(choices: string[], dict: Record<string, string>): string {
	return choices.map(key => dict[key] ?? key).join(' · ');
}

export function getForecastWeekCount(): number {
	return Math.max(FRAME_CHOICES.length, WEAPON_CHOICES.length);
}

export function renderCircuitForecastTable(tbody: HTMLElement, dict: Record<string, string>): void {
