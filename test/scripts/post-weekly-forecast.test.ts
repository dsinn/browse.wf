...
const _worldState = loadMock('worldState.json');
...
const worldState = {
    ..._worldState,
    Descents: _worldState.Descents.map(nextWeek),
    Conquests: _worldState.Conquests.map(nextWeek),
    KnownCalendarSeasons: _worldState.KnownCalendarSeasons.map(nextWeek),
};
...
describe('formatConquest', () => {
    test('returns null when no conquests of the given type exist', () => {
        const result = formatConquest(
            { Conquests: [] }, 'CT_LAB', '/Lotus/Language/Conquest/MissionVariant_LabConquest_',
            'Deep Archimedea', osdict, dict, ExportMissionTypes
        );
    ...
    test('formats Deep Archimedea header with section title only', () => {
        const result = formatConquest(
            worldState, 'CT_LAB', '/Lotus/Language/Conquest/MissionVariant_LabConquest_',
            'Deep Archimedea', osdict, dict, ExportMissionTypes
        )!;
        expect(result).toMatch(/^## Deep Archimedea$/m);
    });

    test('formats Temporal Archimedea header', () => {
        const result = formatConquest(
            worldState, 'CT_HEX', '/Lotus/Language/Conquest/MissionVariant_HexConquest_',
            'Temporal Archimedea', osdict, dict, ExportMissionTypes
        )!;
        expect(result).toMatch(/^## Temporal Archimedea$/m);
    });
    ...
    test('remaps EMPBlackHole condition to MagneticHounds', () => {
        ...
        const result = formatConquest(
            { Conquests: [conquest] }, 'CT_LAB', '/Lotus/Language/Conquest/MissionVariant_LabConquest_',
            'Deep Archimedea', osWithRemap, dict, ExportMissionTypes
        )!;
    ...
    test('Defense mission type becomes DualDefense for CT_LAB', () => {
        ...
        const result = formatConquest(
            { Conquests: [conquest] }, 'CT_LAB', '/Lotus/Language/Conquest/MissionVariant_LabConquest_',
            'Deep Archimedea', osdict, dictWithDefense, ExportMissionTypes
        )!;
        expect(result).toContain('Dual Defense');
    ...
    test('Defense mission type stays Defense for CT_HEX', () => {
        ...
        const result = formatConquest(
            { Conquests: [conquest] }, 'CT_HEX', '/Lotus/Language/Conquest/MissionVariant_HexConquest_',
            'Temporal Archimedea', osdict, dictWithDefense, ExportMissionTypes
        )!;
    ...
    test('each frame variable renders as a bullet with bold name: description', () => {
        ...
        expect(result).toMatch(/^> - \*\*Lethargic Shields\*\*: .+500/m);
    });
    ...
    test('includes Discord timestamp in heading when showTimestamp is true', () => {
        const result = formatConquest(
            worldState, 'CT_LAB', '/Lotus/Language/Conquest/MissionVariant_LabConquest_',
            'Deep Archimedea', osdict, dict, ExportMissionTypes, undefined, true
        )!;
        expect(result).toMatch(/^## Deep Archimedea <t:\d+:D>$/m);
    });
    ...
    test('uses provided find function (findClosest fallback)', () => {
        ...
        const result = formatConquest(
            { Conquests: [staleConquest] }, 'CT_LAB', '/Lotus/Language/Conquest/MissionVariant_LabConquest_',
            'Deep Archimedea', osdict, dict, ExportMissionTypes, findClosestStub, true
        )!;
...
describe('formatDescendia', () => {
    ...
    test('formats header as ## Descendia with no timestamp', () => {
        ...
        expect(result).toMatch(/^## Descendia$/m);
    });
    ...
    test('omits extra separators when specs and auras are empty', () => {
        ...
        expect((challengeLine.match(/ · /g) ?? []).length).toBe(1);
    ...
    test('includes Discord timestamp in heading when showTimestamp is true', () => {
        ...
        expect(result).toMatch(/^## Descendia <t:\d+:D>$/m);
    });
...
describe('formatCalendarSeason', () => {
    ...
    test('formats header with season label, no timestamp', () => {
        const result = formatCalendarSeason(
            worldState, {}, ExportChallenges, ExportResources, ExportBundles, ExportBoosterPacks, ExportBoosters
        )!;
        // worldState mock has CST_FALL
        expect(result).toMatch(/^## 1999 Calendar: 🍁 Autumn$/m);
    });
    ...
    test('challenge with desc+count substitutes |COUNT|', () => {
        ...
        const result = formatCalendarSeason(
            { KnownCalendarSeasons: [season] }, customDict, customChallenges, {}, {}, {}, {}
        )!;
    ...
    test('challenge with count but no desc falls back to camelToWords + count', () => {
        ...
        const result = formatCalendarSeason(
            { KnownCalendarSeasons: [season] }, {}, customChallenges, {}, {}, {}, {}
        )!;
    ...
    test('reward name resolved via item name map and dict', () => {
        ...
        const result = formatCalendarSeason(
            { KnownCalendarSeasons: [season] }, customDict, {}, customResources, {}, {}, {}
        )!;
    ...
    test('includes Discord timestamp in heading when showTimestamp is true', () => {
        ...
        expect(result).toMatch(/^## 1999 Calendar: .+ <t:\d+:D>$/m);
    });
    ...
    test('omits timestamp from heading by default', () => {
        ...
        expect(result).toMatch(/^## 1999 Calendar: .+$/m);
        expect(result).not.toMatch(/<t:\d+:D>/);
    });
});
