...
Promise.all([
    ...
    window.initialProfilePromise,
    fetchExport("ExportImages"),
    window.cloudSyncEvent
]).then(([dict, ExportAchievements, ExportCustoms, ExportEnemies, ExportFactions, ExportFlavour, ExportNightwave, ExportRegions, ExportSentinels, ExportSyndicates, ExportWarframes, ExportWeapons, profile, ExportImages, syncResult]) => {
    window.dict = dict;
...
function renderProfile() {
            ...
            card.className = "card";
            {
                ...
                }
                {
                    ...
                    }
                    {
                        ...
                        if (title) {
                            window.appendSyndicateProgressBar(body, tag, standing, level, title);
                        }
                        else if (tag == "LibrarySyndicate") {
                            window.appendSyndicateProgressBar(body, tag, standing, level, { minStanding: 0, maxStanding: 125_000 });
                        }
                    }
    ...
    for (const stat of ["TimePlayedSec", "Income", "MissionsCompleted", "MissionsFailed", "MissionsQuit", "MissionsInterrupted", "MissionsDumped", "CiphersSolved", "CiphersFailed", "CipherTime", "ReviveCount", "HealCount", "Deaths" /*, "MeleeKills"*/]) {
        ...
        if (stat == "TimePlayedSec" || stat == "CipherTime") {
            ...
            window.addTimeStatTooltip?.(document.getElementById(`stat-${stat}`), stat, value);
        }
        else {
            document.getElementById("stat-" + stat).textContent = value.toLocaleString();
            window.addStatPercentage?.(stat, value);
        }
    }
    if (profile.Stats && profile.Stats.CipherTime && profile.Stats.CiphersSolved) {
        ...
        window.addCipherAvgTooltip?.(document.getElementById("stat-CipherTimeAvg"), profile);
    }
    else {
    ...
    window.augmentEquipmentStats?.(profile);
    ...
    window.augmentEnemyStats?.(profile);
    updateProfileAge();
}
