...
function updateLog() {
    ...
    for (let i = currentHourIndex; i != arbys.length && remainingArbys-- > 0; ++i) {
        ...
        if (!window.isTilesetChecked(node)) {
            continue;
        }
        ...
        window.appendTilesetText(span, node);
        span.textContent += ")";
    ...
    for (let i = currentHourIndex; i != arbys.length && document.querySelector("[data-starved]"); ++i) {
        ...
        window.updateTilesetNextOccurrence(node, arr[0], `${days[thisArbyWeekDay]}, ${months[thisArbyMonth]} ${thisArbyDay}, ${formathour(thisArbyHour)}`, `${toTitleCase(loc(node.missionName))} - ${dict[ExportFactions[node.faction].name]} @ ${loc(node.name)}, ${loc(node.systemName)}`);
    }
...
window.initializeSettingsButtons();
