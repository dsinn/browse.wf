...
describe('Arbitration Timer (/arbys timer badges)', () => {
  ...
  beforeEach(() => {
    ...
    // Set up global references for the script
    ...
    // Load the actual compiled arbys-timer.js
    loadScript('typestripped/src/arbys-timer.js');
  });
  ...
  describe('createArbyCountdownBadge()', () => {
    test('creates a span element with correct attributes', () => {
      ...
      const badge = (window as any).createArbyCountdownBadge(timestamp);

      expect(badge.tagName).toBe('SPAN');
      expect(badge.getAttribute('data-arby-timestamp')).toBe(timestamp.toString());
    ...
    test('shows countdown for future timestamps', () => {
      ...
      const timestamp = Math.floor(now / 1000) + (2 * 86400) + (5 * 3600);
      const badge = (window as any).createArbyCountdownBadge(timestamp);

      expect(badge.textContent).toMatch(/2d \d+h/);
    });
  ...
  describe('Two-unit formatting', () => {
    test('shows days + hours when >= 1 day remaining', () => {
      ...
      const timestamp = Math.floor(now / 1000) + (5 * 86400) + (3 * 3600) + (45 * 60) + 30;
      const badge = (window as any).createArbyCountdownBadge(timestamp);
  ...
  describe('Timer updates with setTimeout', () => {
    test('schedules next update at top of hour for days+hours display', () => {
      ...
      const timestamp = Math.floor(now / 1000) + (2 * 86400) + (30 * 60);
      const badge = (window as any).createArbyCountdownBadge(timestamp);
      ...
      expect(badge.textContent).toMatch(/2d \d+h/); // Still same hour

      // Fast forward to next hour boundary
      vi.advanceTimersByTime(1 * 60 * 1000 + 1);
      expect(badge.textContent).toMatch(/1d 23h|2d \d+h/); // Updated
    });
    ...
    test('schedules next update at top of minute for hours+minutes display', () => {
      ...
      expect(badge.textContent).toMatch(/5h \d+m/);

      // Fast forward just under 30 seconds (shouldn't update yet)
      vi.advanceTimersByTime(29 * 1000);
      expect(badge.textContent).toMatch(/5h \d+m/); // Still same minute

      // Fast forward to next minute boundary
      vi.advanceTimersByTime(1 * 1000 + 1);
      expect(badge.textContent).toMatch(/\d+h \d+m/); // Updated
    });
    ...
    test('schedules next update at top of second for minutes+seconds display', () => {
      ...
      expect(badge.textContent).toMatch(/\d+m \d+s/);
  ...
  describe('initializeArbyTimer()', () => {
    test('initializes timers for all existing badges', () => {
      ...
      badge1.setAttribute('data-arby-timestamp', timestamp1.toString());
      document.body.appendChild(badge1);

      const badge2 = document.createElement('span');
      badge2.setAttribute('data-arby-timestamp', timestamp2.toString());
      document.body.appendChild(badge2);

      // Initialize timers
      (window as any).initializeArbyTimer();

      // Both badges should have been updated with countdown text
      expect(badge1.textContent).toMatch(/\d+m/);
      expect(badge2.textContent).toMatch(/\d+h/);
    });
  ...
  describe('Edge cases', () => {
    test('handles exactly 1 day remaining', () => {
      ...
      const timestamp = Math.floor(now / 1000) + 86400;
      const badge = (window as any).createArbyCountdownBadge(timestamp);
    ...
    test('handles 99+ days correctly', () => {
      ...
      // 150 days
      const timestamp = Math.floor(now / 1000) + (150 * 86400);
      const badge = (window as any).createArbyCountdownBadge(timestamp);

      expect(badge.textContent).toMatch(/150d \d+h/);
    });
  });
});
