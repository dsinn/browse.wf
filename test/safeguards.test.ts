...
describe('Production Domain Safeguards', () => {
	...
	describe('Vitest fetch mocking', () => {
		...
		it('blocks unmocked oracle.browse.wf endpoints', async () => {
			...
			await expect(fetch('https://oracle.browse.wf/unknown-endpoint')).rejects.toThrow(/TEST SAFEGUARD.*production domain/u);
		});

		it('blocks browse.wf endpoints', async () => {
			// Arbys.txt is mocked, but other endpoints should be blocked
			await expect(fetch('https://browse.wf/unknown-file.txt')).rejects.toThrow(/TEST SAFEGUARD.*production domain/u);
		});
		...
		it('allows requests to non-blocked domains', async () => {
			...
			await expect(fetch('https://example.com/api')).rejects.toThrow(/No mock/u);

			// The error should NOT be the safeguard error
			await expect(fetch('https://example.com/api')).rejects.not.toThrow(/TEST SAFEGUARD/u);
		});
	});
	...
	describe('Real-world URL patterns', () => {
		it('blocks worldState.json if accidentally called with wrong URL', async () => {
			...
			const fetchPromise = fetch('https://oracle.browse.wf/worldstate.json'); // Lowercase 's'
			await expect(fetchPromise).rejects.toThrow(/TEST SAFEGUARD/u);
		});

		it('blocks API calls with query parameters', async () => {
			await expect(fetch('https://oracle.browse.wf/bounty-cycle?timestamp=12345')).rejects.toThrow(/TEST SAFEGUARD/u);
		});
