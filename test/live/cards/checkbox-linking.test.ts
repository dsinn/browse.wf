...
beforeEach(() => {
    ...
    // Minimal mocks
    const oids_completed: string[] = [];
    (window as any).isOidMarkedAsCompleted = (oid: string) => oids_completed.includes(oid);
    (window as any).setCompletionToggle = (el: HTMLAnchorElement, completed: boolean) => {
        const oid = el.getAttribute("data-oid")!;
        const idx = oids_completed.indexOf(oid);
        if (completed && idx === -1) {
            oids_completed.push(oid);
