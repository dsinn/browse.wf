...
describe('createArchimedeaTooltipEl', () => {
  const osdict = {
  ...
  test('returns <abbr> with tooltip when text and desc exist', () => {
    ...
    expect(el.nodeName).toBe('ABBR');
    expect(el.textContent).toBe('Bold Venture');
    expect(el.getAttribute('data-bs-toggle')).toBe('tooltip');
    expect(el.getAttribute('data-bs-title')).toBe('Enemies deal less damage.');
  });
  ...
  test('returns text node when only text key exists (no _Desc)', () => {
    ...
    expect(el.textContent).toBe('Has Name Only');
  });
  ...
  test('returns raw value text node when key is missing entirely', () => {
    ...
    expect(el.nodeType).toBe(Node.TEXT_NODE);
    expect(el.textContent).toBe('CompletelyUnknownTag');
  });
});
