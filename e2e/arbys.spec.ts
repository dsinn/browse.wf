...
test.describe('Arbitration Schedule (/arbys)', () => {
	...
	test.describe('Dropdown interactions', () => {
		...
		test('changing hour format updates time display', async ({page}) => {
			...
			await expect(firstEntry).toContainText(/\d{4}/u); // Wait for 4-digit time
			let text = await firstEntry.textContent();
			expect(text).toMatch(/\d{4}/u); // Should have 4-digit time

			// Test 24-hour time
			await page.selectOption('#select-hourfmt', '24');
			await expect(firstEntry).toContainText(/\d{2}:\d{2}/u); // Wait for colon format
			text = await firstEntry.textContent();
			expect(text).toMatch(/\d{2}:\d{2}/u); // Should have colon format

			// Test 12-hour time
			await page.selectOption('#select-hourfmt', '12');
			await expect(firstEntry).toContainText(/\d{1,2}(am|pm)/u); // Wait for am/pm
			text = await firstEntry.textContent();
			expect(text).toMatch(/\d{1,2}(am|pm)/u); // Should have am/pm
		});
	});
	...
	test.describe('URL fragment parameters', () => {
		test('loads with days parameter from URL', async ({page}) => {
			...
			await page.evaluate(() => {
				globalThis.location.hash = 'days=7';
			});
			await page.reload();
			await page.waitForSelector('#log:not(:has-text("Loading, please wait..."))');
		...
		test('loads with timezone parameter from URL', async ({page}) => {
			...
			await page.evaluate(() => {
				globalThis.location.hash = 'tz=zulu';
			});
			await page.reload();
			await page.waitForSelector('#log:not(:has-text("Loading, please wait..."))');
			...
			expect(firstEntry).toMatch(/\d{4}Z •/u);
		});
		...
		test('loads with hour format parameter from URL', async ({page}) => {
			...
			await page.evaluate(() => {
				globalThis.location.hash = 'hourfmt=12';
			});
			await page.reload();
			await page.waitForSelector('#log:not(:has-text("Loading, please wait..."))');
			...
			const firstEntry = await page.locator('#log [data-timestamp]').first().textContent();
			expect(firstEntry).toMatch(/\d{1,2}(am|pm)/u);
		});
		...
		test('loads with exclude parameter unchecking filters', async ({page}) => {
			...
			await page.evaluate(() => {
				globalThis.location.hash = 'exclude=MT_SURVIVAL.MT_DEFENSE';
			});
			await page.reload();
			await page.waitForSelector('#log:not(:has-text("Loading, please wait..."))');
		...
		test('loads with combined parameters', async ({page}) => {
			...
			await page.evaluate(() => {
				globalThis.location.hash = 'days=90&tz=zulu&hourfmt=24&exclude=MT_SURVIVAL';
			});
			await page.reload();
			await page.waitForSelector('#log:not(:has-text("Loading, please wait..."))');
			...
			const firstEntry = await page.locator('#log [data-timestamp]').first().textContent();
			expect(firstEntry).toMatch(/\d{2}:\d{2}/u);
			expect(firstEntry).not.toMatch(/\d{2}Z/u);
		});
	});
	...
	test.describe('Next occurrence table', () => {
		test('populates with correct data for mission types', async ({page}) => {
			...
			if (dateText !== 'N/A') {
				expect(dateText).toMatch(/(Sun|Mon|Tue|Wed|Thu|Fri|Sat)/u);
	...
	test.describe('Log display', () => {
		test('groups arbitrations by day with headers', async ({page}) => {
			...
			expect(firstHeaderText).toMatch(/(Sun|Mon|Tue|Wed|Thu|Fri|Sat), (January|February|March|April|May|June|July|August|September|October|November|December) \d+/u);
		});
		...
		test('shows entries with timestamps in chronological order', async ({page}) => {
			...
			for (const entry of entries) {
				...
				if (ts) {
					timestamps.push(Number.parseInt(ts, 10));
				}
			}
	...
	test.describe('Timer badges', () => {
		...
		test('have correct styling and fixed width', async ({page}) => {
			...
			await expect(firstBadge).toHaveClass(/text-bg-secondary/u);

			// Check fixed width and inline display
			const width = await firstBadge.evaluate(element => globalThis.getComputedStyle(element).width);
			const display = await firstBadge.evaluate(element => globalThis.getComputedStyle(element).display);
	...
	test.describe('Save/Load settings buttons', () => {
		...
		test('Save button saves settings to localStorage and enables Load button', async ({page}) => {
			...
			await expect(saveBtn).toContainText(/Sav(ing|ed)/u);
