...
describe('Incursions Card - DOM Structure', () => {
  ...
  test('empty message element exists and is hidden by default', () => {
    const msg = getById('incursions-empty-message');
  ...
  test('filter panel has checkboxes for all expected mission types', () => {
    const expectedTypes = [
      'MT_ALCHEMY', 'MT_ASCENSION', 'MT_ASSASSINATION', 'MT_ASSAULT',
      'MT_CAPTURE', 'MT_EVACUATION', 'MT_DEFENSE', 'MT_ARTIFACT',
      'MT_EXCAVATE', 'MT_EXTERMINATION', 'MT_RETRIEVAL', 'MT_HIVE',
      'MT_PURIFY', 'MT_TERRITORY', 'MT_MOBILE_DEFENSE', 'MT_PURSUIT',
      'MT_RESCUE', 'MT_RUSH', 'MT_SABOTAGE', 'MT_OFFERING',
      'MT_SPY', 'MT_SURVIVAL', 'MT_ARMAGEDDON', 'MT_VOID_CASCADE', 'MT_CORRUPTION',
    ];
