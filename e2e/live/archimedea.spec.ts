...
async function setupPage(page: any, worldStateFile?: string) {
  ...
  await page.waitForSelector('#labConquest-missions:not(:has-text("Fetching data..."))', { timeout: 15000 });
  await page.waitForSelector('#hexConquest-missions:not(:has-text("Fetching data..."))', { timeout: 15000 });
}
...
async function setupPageWithManualWeeklyUpdate(page: any, worldStateFile: string) {
  ...
  await page.evaluate(() => {
    if (typeof (window as any).updateWeekly === 'function') {
      (window as any).updateWeekly();
    }
  });

  // Wait for cards to populate
  await page.waitForSelector('#labConquest-missions:not(:has-text("Fetching data..."))', { timeout: 10000 });
  await page.waitForSelector('#hexConquest-missions:not(:has-text("Fetching data..."))', { timeout: 10000 });
}
...
test.describe('Live Page - Archimedea Cards', () => {
  ...
  test.describe('Deep Archimedea Card', () => {
    test('renders card header with expiry', async ({ page }) => {
      ...
      await page.waitForSelector('#labConquest-header', { timeout: 10000 });
    ...
    test('renders exactly 3 missions', async ({ page }) => {
      ...
      await page.waitForSelector('#labConquest-missions tbody tr', { timeout: 10000 });
    ...
    test('displays frame variables (modifiers)', async ({ page }) => {
      await page.waitForSelector('#labConquest-fv', { timeout: 10000 });
  ...
  test.describe('Temporal Archimedea Card', () => {
    test('renders card header with expiry', async ({ page }) => {
      // Wait for card to render
      await page.waitForSelector('#hexConquest-header', { timeout: 10000 });
    ...
    test('renders exactly 3 missions', async ({ page }) => {
      // Wait for missions table to render
      await page.waitForSelector('#hexConquest-missions tbody tr', { timeout: 10000 });
    ...
    test('displays frame variables (modifiers)', async ({ page }) => {
      await page.waitForSelector('#hexConquest-fv', { timeout: 10000 });
  ...
  test.describe('Data Transformation', () => {
    test('Deep and Temporal Archimedea have distinct mission types', async ({ page }) => {
      ...
      labTypes.forEach(type => expect(type).toBeTruthy());
      hexTypes.forEach(type => expect(type).toBeTruthy());
...
test.describe('Special Cases - CT_LAB Defense Mission', () => {
  ...
  test('transforms Defense mission to DualDefense', async ({ page }) => {
    ...
    expect(firstMissionType).toMatch(/Defense/i);
...
test.describe('Special Cases - Unknown Frame Variable', () => {
  test('shows raw value without tooltip and logs console warning', async ({ page }) => {
    ...
    page.on('console', msg => {
      if (msg.type() === 'warning' && msg.text().includes('Missing osdict key')) {
