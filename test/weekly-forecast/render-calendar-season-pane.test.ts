...
describe('renderCalendarSeasonPane', () => {
  ...
  describe('Global exposure', () => {
    test('renderCalendarSeasonPane is exposed as a global function', async () => {
      expect(typeof (window as any).renderCalendarSeasonPane).toBe('function');
    });

    test('getSeasonLabel is exposed as a global function', async () => {
      expect(typeof (window as any).getSeasonLabel).toBe('function');
    });

    test('updateCalendarSeason is exposed as a global function', async () => {
      expect(typeof (window as any).updateCalendarSeason).toBe('function');
    });
  });

  describe('Return value', () => {
    test('returns a <div> element', async () => {
      const pane = await (window as any).renderCalendarSeasonPane(season, ...defaultExportPromises);
  ...
  describe('Day filtering', () => {
    test('days with no events are not rendered', async () => {
      ...
      const pane = await (window as any).renderCalendarSeasonPane(season, ...defaultExportPromises) as HTMLDivElement;
  ...
  describe('Date formatting', () => {
    ...
    test('day 1 formats to "Jan 1"', async () => {
      ...
      const pane = await (window as any).renderCalendarSeasonPane(testSeason, ...defaultExportPromises) as HTMLDivElement;
  ...
  describe('Date column emoji prefix', () => {
    test('CET_CHALLENGE day shows 📋 prefix in date column', async () => {
      const pane = await (window as any).renderCalendarSeasonPane(season, ...defaultExportPromises) as HTMLDivElement;
      const firstDate = pane.querySelector('.calendar-season-date');
    ...
    test('CET_REWARD day shows 🎁 prefix in date column', async () => {
      const testSeason = {
        ...season,
    ...
    test('CET_UPGRADE day shows 🔧 prefix in date column', async () => {
      const testSeason = {
        ...season,
        Days: [{ day: 1, events: [{ type: 'CET_UPGRADE', upgrade: '/Lotus/Upgrades/Test' }] }]
      };
  ...
  describe('Reward events (CET_REWARD)', () => {
    ...
    test('normalizes /Lotus/StoreItems/ → /Lotus/ before looking up in itemIconMap/itemNameMap', async () => {
  ...
  describe('Upgrade events (CET_UPGRADE)', () => {
    test('renders a span with textContent "✨"', async () => {
      const testSeason = {
        ...season,
      ...
      // First upgrade event should have ✨
...
describe('getSeasonLabel', () => {
  describe('Known seasons', () => {
    test('CST_SPRING returns "🌸 Spring"', () => {
      const label = (window as any).getSeasonLabel('CST_SPRING');
      expect(label).toBe('🌸 Spring');
    });

    test('CST_SUMMER returns "🌻 Summer"', () => {
      const label = (window as any).getSeasonLabel('CST_SUMMER');
      expect(label).toBe('🌻 Summer');
    });

    test('CST_FALL returns "🍁 Autumn"', () => {
      const label = (window as any).getSeasonLabel('CST_FALL');
      expect(label).toBe('🍁 Autumn');
    });

    test('CST_WINTER returns "❄️ Winter"', () => {
      const label = (window as any).getSeasonLabel('CST_WINTER');
  ...
  describe('Unknown season', () => {
    test('unknown key returns the raw value unchanged', () => {
      const label = (window as any).getSeasonLabel('CST_UNKNOWN');
