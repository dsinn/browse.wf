(() => {
  ...
  var handler;
  var debounceTimer;
  function registerSyncHandler(fn) {
    handler = fn;
  }
  function triggerCloudSync() {
    void handler?.();
  }
  function triggerCloudSyncWithDebounce() {
  ...
  async function waitForCloudSync() {
  ...
  function saveToLocalStorage() {
    ...
    for (const checkbox of document.querySelectorAll('input[type=checkbox][id^="filter-"]')) {
      const filterId = checkbox.id.slice(7);
  ...
  function updateTilesetNextOccurrence(node, timestamp, dateText, detailText) {
    const tileset = window.getTileset(node);
    if (!tileset) {
      return;
    }
  ...
  var fetchExportImpl = isBrowser ? fetchExportBrowser : fetchExportNode;
  ...
  function pascalToTitleCase(string_) {
  ...
  var CONQUEST_RISK_REMAP = { EMPBlackHole: "MagneticHounds" };
  var CONQUEST_VARIABLE_REMAP = { DullBlades: "ComboCountChance", Undersupplied: "MaxAmmo" };
  var FRAME_VARIABLE_VALUE_MAP = { ShieldDelay: "500", TimeDilation: "50" };
  ...
  async function resolveArchimedea(archimedea, archimedeaType, variantKeyPrefix, osdict, dict) {
    ...
    for (const mission of archimedea.Missions) {
      ...
      const conditions = hardDiff.risks.slice(0, 2).map((r) => {
        ...
        return {
          ...
          desc: osdict[condKey + "_Desc"] ? stripHtml(osdict[condKey + "_Desc"]) : void 0
        };
      });
      missions.push({
        type: rawTypeName,
        variant: variantName,
        variantDesc: variantDescRaw ? stripHtml(variantDescRaw) : void 0,
        conditions
      });
    }
  ...
  function renderArchimedeaMissions(missions) {
  ...
  function renderArchimedeaFrameVariables(frameVariables) {
    const tr = document.createElement("tr");
  ...
  async function renderArchimedeaTable(container, archimedea, archimedeaType, variantKeyPrefix) {
  ...
  var SEASON_LABELS = {
    CST_SPRING: "\u{1F338} Spring",
    CST_SUMMER: "\u{1F33B} Summer",
    CST_FALL: "\u{1F342} Autumn",
    CST_WINTER: "\u2744\uFE0F Winter"
  };
  var EVENT_EMOJI = {
    CET_CHALLENGE: "\u{1F4CB}",
    CET_REWARD: "\u{1F381}",
    CET_UPGRADE: "\u{1F527}"
  };
  ...
  function resolveChallengeText(event, dict, exportChallenges) {
    ...
    if (!challengeData) {
      return { text: camelToWords2(lastSegment(event.challenge)) };
    }
    const iconPath = challengeData.icon || void 0;
    ...
    if (count) {
      return { text: `${camelToWords2(lastSegment(event.challenge))} \xD7${String(count)}`, iconPath };
    }
    return { text: camelToWords2(lastSegment(event.challenge)), iconPath };
  }
  var itemMapsPromise;
  async function fetchItemMaps() {
    if (itemMapsPromise === void 0) {
  ...
  async function resolveCalendarSeasonDays(season, dict) {
      ...
      for (const event of dayEntry.events) {
        ...
        switch (event.type) {
          ...
          case "CET_REWARD": {
            ...
            const rawName = (nameKey && dict[nameKey]) ?? camelToWords2(lastSegment(event.reward));
            text = rawName.replaceAll(/<[^>]+>\s*/gu, "").trim();
            break;
          }
          case "CET_UPGRADE": {
            text = camelToWords2(lastSegment(event.upgrade));
            break;
          }
        }
        if (text) {
  ...
  if (globalThis.window !== void 0) {
    ...
    window.camelToWords = camelToWords2;
  ...
  async function renderCalendarSeasonPane(season) {
  ...
  // src/card-filters.ts
  function refreshFilterStatus(elm) {
  ...
  function initializeFilterToggles() {
    for (const elm of document.querySelectorAll("[data-filter-toggle]")) {
  ...
  function initializeCardFilters(cardName, onFilterChange) {
    for (const checkbox of document.querySelectorAll(`#${cardName}-filters input[type=checkbox]`)) {
  ...
  function isFilterEnabled(cardName, filterType) {
  ...
  function getNextWeeklyResetSeconds() {
    return getNextWeeklyResetMs() / 1e3;
  }
  var SECONDS_PER_HOUR = 60 * 60;
  var SECONDS_PER_DAY = 24 * SECONDS_PER_HOUR;
  var SECONDS_PER_WEEK = 7 * SECONDS_PER_DAY;
  var MILLIS_PER_HOUR = SECONDS_PER_HOUR * 1e3;
  var MILLIS_PER_DAY = SECONDS_PER_DAY * 1e3;
  var MILLIS_PER_WEEK = SECONDS_PER_WEEK * 1e3;
  var WEEK_EPOCH_MS = 13919904e5;
  function getWeekIndex(timestamp) {
    return Math.trunc((timestamp - WEEK_EPOCH_MS) / MILLIS_PER_WEEK);
  }
  function totwo(number_) {
    if (number_ < 10) {
      return "0" + number_;
    }
    return number_.toString();
  }
  function formattz(offsetMinutes) {
    if (offsetMinutes === 0) {
      return "UTC+0";
    }
  ...
  // src/clan-weekly/data.ts
  function lastSegment2(path) {
    return path.split("/").pop() ?? path;
  }
  function resolveBonusRegion(bonusRegion, dict) {
    return dict[bonusRegion] ?? dict[bonusRegion + "_SPACE"] ?? camelToWords2(lastSegment2(bonusRegion));
  }
  function findClanWeeklyEntry(entries) {
    const currentWeekIdx = getWeekIndex(Date.now());
    return entries.find((entry) => entry.WeekCount === currentWeekIdx) ?? entries.find((entry) => entry.WeekCount === currentWeekIdx + 1);
  }
  var itemMapsPromise2;
  async function fetchItemMaps2() {
    if (itemMapsPromise2 === void 0) {
      itemMapsPromise2 = (async () => {
    ...
    return itemMapsPromise2;
  }
  async function resolveClanWeeklyRewards(entry, dict) {
    const { itemNameMap, itemIconMap, fusionPointsMap } = await fetchItemMaps2();
    ...
    return sorted.map((reward) => {
      ...
      if (normalized.includes("/PickUps/Credits/")) {
        const denomination = Number.parseInt(lastSegment2(reward.Reward).replaceAll(/\D+/gu, ""), 10);
      ...
      const nameKey = itemNameMap[normalized];
      const itemName = (nameKey && dict[nameKey]) ?? camelToWords2(lastSegment2(reward.Reward));
      return { pct, display: `${reward.ItemCount.toLocaleString("en")}x ${itemName}`, iconPath };
    });
  }

  // src/clan-weekly/index.ts
  function makeIcon2(iconPath) {
    const img = document.createElement("img");
    img.style.height = "24px";
    img.style.width = "24px";
    img.style.objectFit = "contain";
  ...
  async function renderClanWeeklyPane(entry) {
    const dict = await getDictPromise();
    const container = document.createElement("div");
    ...
    for (const reward of rewards) {
      ...
      if (reward.iconPath) {
        rewardSpan.append(makeIcon2(reward.iconPath));
      }
  ...
  async function updateClanWeekly() {
  ...
  // src/logger.ts
  var isProduction = () => window.__ENV__?.VITE_ENV === "production";
  var formattedTimestamp = () => `[${(/* @__PURE__ */ new Date()).toISOString()}]`;
  var logger = {
    /**
...
${suffix}`;
  ...
  var GoTrueClient = class _GoTrueClient {
    ...
    async _exchangeCodeForSession(authCode, options) {
      ...
      const requestedFlowId = hasExplicitFlowId ? validatePKCEFlowId(options === null || options === void 0 ? void 0 : options.flowId) : isBrowser2() ? validatePKCEFlowId(parseParametersFromURL(window.location.href)[PKCE_FLOW_ID_PARAM]) : null;
    ...
    async signInWithSSO(params) {
      ...
      try {
        ...
        if (((_c = result.data) === null || _c === void 0 ? void 0 : _c.url) && isBrowser2() && !((_d = params.options) === null || _d === void 0 ? void 0 : _d.skipBrowserRedirect)) {
  ...
  // src/cloud-sync/database.ts
  var databaseUrl = window.__ENV__?.VITE_DATABASE_URL;
  var databaseKey = window.__ENV__?.VITE_DATABASE_ANON_KEY;
  var db = databaseUrl && databaseKey ? createClient(databaseUrl, databaseKey, {
    auth: {
      persistSession: true,
      autoRefreshToken: true
    }
  ...
  var isDatabaseConfigured = () => Boolean(databaseUrl && databaseKey);

  // src/cloud-sync/auth.ts
  var AuthService = class _AuthService {
    ...
    async initialize() {
      ...
      this.currentUser = user ?? void 0;
      db.auth.onAuthStateChange((event, session) => {
        this.currentUser = session?.user ?? void 0;
        setTimeout(() => {
    ...
    isAuthenticated() {
      return this.currentUser !== void 0;
    }
    getUserId() {
      return this.currentUser?.id;
    }
    getDiscordUserId() {
      return this.currentUser?.user_metadata?.provider_id || void 0;
    }
  ...
  // src/cloud-sync/serializer.ts
  var localOnlyKeyRegex = /^(?:sb-.*-auth-token|profile\.data)/u;
  ...
  // src/cloud-sync/handler.ts
  var CloudSyncHandler = class {
    constructor() {
      this.syncing = false;
    ...
    async pushToDatabase(userId) {
      ...
      if (this.justPushedTimeout !== void 0) {
    ...
    async pullFromDatabase(userId) {
      const { data: row, error } = await db.from("user_data").select("data").eq("user_id", userId).single();
      if (error) {
  ...
  // src/cloud-sync/health-monitor.ts
  var CloudSyncHealthMonitor = class _CloudSyncHealthMonitor {
    ...
    startHeartbeat(getChannelState, setLastFreshTimestamp) {
      globalThis.setInterval(() => {
        if (getChannelState() === "joined") {
          ...
          logger.debug("\u{1F493} Heartbeat: Connection healthy");
        }
      }, _CloudSyncHealthMonitor.heartbeatIntervalMs);
    }
    /**
    ...
    startVisibilityFallback(options) {
      ...
      document.addEventListener("visibilitychange", () => {
        void (async () => {
          ...
          if (document.hidden || !getCurrentUserId() || isSyncing() || getChannelState() === "joined" || timeSinceLastFreshMs < _CloudSyncHealthMonitor.visibilityPullThrottleMs) {
            return;
          }
          ...
          try {
            ...
            logger.log("\u2601\uFE0F\u27A1\uFE0F\u{1F4BB} Synced data from cloud (visibility fallback)");
          } catch (error) {
  ...
  // src/cloud-sync/subscription.ts
  var CloudSyncSubscription = class {
    ...
    subscribe(userId) {
      ...
      this.channel = db.channel(`user_data:${userId}`).on("postgres_changes", {
        event: "UPDATE",
        schema: "public",
        table: "user_data",
        ...
        if (!this.syncHandler.syncing) {
          try {
            ...
            logger.log("\u2601\uFE0F\u27A1\uFE0F\u{1F4BB} Synced data from cloud");
          } finally {
  ...
  // src/cloud-sync/reconnection.ts
  var CloudSyncReconnection = class _CloudSyncReconnection {
    constructor(onReconnect, getCurrentUserId) {
      this.onReconnect = onReconnect;
      this.getCurrentUserId = getCurrentUserId;
      this.reconnectAttempts = 0;
    }
    static get initialBackoffMs() {
      return 1875;
    }
    static get maxBackoffMs() {
      return 5 * 60 * 1e3;
    }
    static get errorThresholdMs() {
      return 60 * 1e3;
    }
    get attempts() {
    ...
    get lastFreshTimestamp() {
      return this.lastKnownFreshDataTimestamp;
    }
    set lastFreshTimestamp(value) {
    ...
    clearAll() {
      this.reset();
      this.lastKnownFreshDataTimestamp = void 0;
    }
    /**
    ...
    attemptReconnect() {
      ...
      const delayMs = Math.min(
        _CloudSyncReconnection.initialBackoffMs * 2 ** this.reconnectAttempts,
        _CloudSyncReconnection.maxBackoffMs
      );
      this.reconnectAttempts++;
      const lastConnectedMessage = this.lastKnownFreshDataTimestamp ? `Last connected: ${new Date(this.lastKnownFreshDataTimestamp).toLocaleTimeString()}` : "Never successfully connected";
      if (delayMs >= _CloudSyncReconnection.errorThresholdMs) {
        const minutes = delayMs / 6e4;
        ...
        logger.debug(`\u{1F504} WebSocket disconnected. Reconnecting in ${delayMs / 1e3}s (attempt ${this.reconnectAttempts}). ${lastConnectedMessage}`);
      }
      this.reconnectTimer = globalThis.setTimeout(() => {
        ...
        void (async () => {
          ...
          if (userId) {
            logger.debug("\u{1F50C} Attempting to reestablish WebSocket connection...");
            try {
  ...
  // src/cloud-sync/manager.ts
  var CloudSyncManager = class _CloudSyncManager {
    // Track current user UUID for reconnection
    constructor() {
      this.loginSyncComplete = false;
      this.handler = new CloudSyncHandler();
      this.healthMonitor = new CloudSyncHealthMonitor();
      this.reconnection = new CloudSyncReconnection(
        (userId) => {
          this.subscribeToRealtimeUpdates(userId);
        },
        () => this.currentUserId
      );
      this.subscription = new CloudSyncSubscription(
        this.handler,
        () => {
    ...
    static getInstance() {
      _CloudSyncManager.instance ||= new _CloudSyncManager();
      return _CloudSyncManager.instance;
    }
    /**
    ...
    async handleLogin() {
      ...
      try {
        ...
        const { data: remoteData, error } = await db.from("user_data").select("data").eq("user_id", userId).single();
        if (error || !remoteData) {
          await this.pushToDatabase(userId);
          logger.log("\u{1F4BB}\u27A1\uFE0F\u2601\uFE0F Your data has been backed up to the cloud");
        } else {
    ...
    async onSubscribed(isResubscribe) {
      ...
      logger.debug("\u{1F4E1} WebSocket SUBSCRIBED | isResubscribe:", isResubscribe, "| reconnectAttempts:", attempts);
      if (attempts > 0) {
      ...
      if (isResubscribe) {
        ...
        if (timeSinceLastFreshMs <= CloudSyncHealthMonitor.quickReconnectThresholdMs) {
          logger.debug(`\u26A1 Reconnected with recent heartbeat (${(timeSinceLastFreshMs / 1e3).toFixed(1)}s ago) - skipping pull`);
          return;
        }
        logger.debug(`\u{1F504} Reconnected${lastFreshTs ? ` with stale heartbeat (${(timeSinceLastFreshMs / 1e3).toFixed(1)}s ago)` : ""} - pulling fresh data`);
        if (this.currentUserId && !this.handler.syncing) {
          try {
            this.handler.syncing = true;
            await this.pullFromDatabase(this.currentUserId);
            logger.log("\u2601\uFE0F\u27A1\uFE0F\u{1F4BB} Synced data from cloud (reconnected)");
          } finally {
            this.handler.syncing = false;
          }
        }
    ...
    startHeartbeat() {
      this.healthMonitor.startHeartbeat(
        () => this.subscription.channelState,
        (ts) => {
  ...
  function generateGenericAvatar() {
  ...
  registerSyncHandler(async () => {
    ...
    if (userId) {
      try {
        ...
        logger.log("\u{1F4BB}\u27A1\uFE0F\u2601\uFE0F Synced data to cloud");
      } catch (error) {
  ...
  var ARENA_EMOJI = {
    ArenaAvocado: "\u{1F951}",
    ArenaBagel: "\u{1F96F}",
    ArenaCherry: "\u{1F352}",
    ArenaCoconut: "\u{1F965}",
    ArenaEggplant: "\u{1F346}",
    ArenaGrape: "\u{1F347}",
    ArenaMango: "\u{1F96D}",
    ArenaMelon: "\u{1F348}",
    ArenaPeach: "\u{1F351}",
    ArenaWaffle: "\u{1F9C7}",
    BossArenaSmall: "\u{1F419}",
    BossArenaUriel: "\u{1F608}",
    ProtoframeRoomHarrow: "\u{1F472}\u{1F3FC}",
    ProtoframeRoomWisp: "\u{1F470}\u{1F3FC}\u200D\u2640\uFE0F",
    SpecialChallengeArena01: "\u{1F434}",
    SpecialChallengeArena02: "\u{1F434}",
    SpecialChallengeArena03: "\u{1F434}"
  };
  function resolveDescentChallenges(descent, dict) {
    const tail = (p) => p.split("/").pop() ?? p;
    const toWords = (s) => s.replaceAll(/(?<=[a-z])(?=[A-Z])|(?<=.)(?=[A-Z](?:[a-z]|$))/gu, " ");
    const toTitleCase2 = (s) => s.replaceAll(/\w+/gu, (w) => w[0].toUpperCase() + w.slice(1).toLowerCase());
    ...
    return descent.Challenges.map((ch) => {
      ...
      const rawTypeLabel = toTitleCase2(ch.Type.replace(/^DT_/u, "").replaceAll("_", " "));
  ...
  function renderDescentChallenges(descent, dict) {
    const tbody = document.createElement("tbody");
    for (const row of resolveDescentChallenges(descent, dict)) {
      ...
      {
        const td = document.createElement("td");
        td.textContent = [...row.specs, ...row.auras].join("\xA0\xB7 ") || "\u2014";
        tr.append(td);
      }
      tbody.append(tr);
    }
  ...
  function getTilesetPrefixes() {
    if (!tilesetPrefixes) {
      const knownTilesets = [...new Set(Object.values(window.ExportRegions).map((n) => n.tileset).filter((t) => t !== void 0))];
      tilesetPrefixes = knownTilesets.sort((a, b) => b.length - a.length);
    }
    return tilesetPrefixes;
  }
  function getTileset(node) {
    if (node.tileset) {
      return node.tileset;
    }
  ...
  function formatTileset(tileset) {
    if (!tileset) {
      return "";
    }
    return tileset.replace("Tileset", "").replaceAll(/(?<=[a-z])(?=[A-Z])/gu, " ");
  }
  ...
  function formatShortTimerCountdown(timestamp, expiredLabel) {
    const deltaSeconds = timestamp - Math.floor(Date.now() / 1e3);
    if (deltaSeconds <= 0) {
      return expiredLabel;
    }
  ...
  function scheduleShortTimerUpdate(elm) {
    ...
    if (deltaSeconds >= SECONDS_PER_DAY) {
      const delayMs = (SECONDS_PER_HOUR - deltaSeconds % SECONDS_PER_HOUR) * 1e3;
      setTimeout(() => {
        scheduleShortTimerUpdate(elm);
      }, delayMs);
    } else if (deltaSeconds >= SECONDS_PER_HOUR) {
      const delayMs = (60 - deltaSeconds % 60) * 1e3;
      setTimeout(() => {
        scheduleShortTimerUpdate(elm);
      }, delayMs);
    } else {
  ...
  function createShortTimerBadge(timestamp, expiredLabel, extraClasses) {
    const span = document.createElement("span");
  ...
  var invigorationNames = {};
  function initInvigorations(names) {
  ...
  function populateInvigorationGrid(prefix, response, suits) {
    const { baseSuitTypes } = globalThis;
    const { dict } = globalThis;
  ...
  function preFillForm(username, peek, suits) {
    ...
    const peekCheckbox = document.querySelector("#peek");
  ...
  function saveToCache(request2, response) {
    const currentWeek = getWeekIndex(Date.now());
    const targetWeek = request2.p ? currentWeek + 1 : currentWeek;
    const cache = loadCache();
    cache[targetWeek] = { request: request2, response };
    const prunedCache = {};
  ...
  function showHistory(currentWeek, cache) {
    ...
    if (currentData) {
      const thisWeekSuits = nextWeekData ? nextWeekData.request.s : void 0;
    ...
    if (lastWeekData) {
      const lastWeekSuits = currentData ? currentData.request.s : void 0;
  ...
  function showResults(response, request2) {
    const resultsDiv = document.querySelector("#results");
    resultsDiv?.classList.remove("d-none");
    const heading = document.querySelector("#results h4");
    if (heading) {
      heading.textContent = request2.p ? "Next Week's Offerings" : "Current Offerings";
    }
    for (const x of document.querySelectorAll(".explainer")) {
      x.classList.add("d-none");
    }
    if (request2.s.length !== response.suits.length) {
    ...
    for (const x of document.querySelectorAll("#results b")) {
      x.textContent = request2.n;
    }
    populateInvigorationGrid("out", response);
  }
  function initInvigorationTimer() {
  ...
  function scheduleInvigorationReload() {
    const weekEnd = getNextWeeklyResetSeconds();
    setTimeout(() => {
      location.reload();
    }, weekEnd * 1e3 - Date.now());
  }
  function showCacheAlert(type, message) {
    const alert2 = document.querySelector("#cache-alert");
    if (!alert2) {
      return;
    }
    alert2.className = `alert alert-${type} mb-3`;
    alert2.textContent = message;
    alert2.classList.remove("d-none");
  }
  function initInvigorationsFromCache(inventoryDataUsed) {
  ...
  var dailyResetTimer;
  function updateBountyCheckboxes() {
  ...
  function renderAllyIcon(allyName, allyCell) {
  ...
  function applyBountyFilters(syndicateTag, rows) {
    ...
    for (const [i, row] of [...rows].entries()) {
      const tier = i + 1;
      const missionType = row.dataset.missionType ?? "";
  ...
  function isBountyMissionTypeEnabled(syndicateTag, missionType) {
    const storageKey = `live.filter.bounties.${syndicateTag}.${missionType}`;
    return localStorage.getItem(storageKey) !== "0";
  }
  function getMinimumTier(syndicateTag) {
  ...
  function initializeDeimosFilter() {
    const checkbox = document.querySelector("#bounty-filter-deimos");
    if (!checkbox) {
      return;
    }
  ...
  function initializeBountyFilters() {
    initializeDeimosFilter();
    for (const syndicateTag of SYNDICATE_TAGS) {
      const select = document.querySelector(`#bounty-filter-${syndicateTag}`);
      if (!select) {
        continue;
      }
      ...
      const checkboxes = document.querySelectorAll(`input[type="checkbox"][data-bounty-syndicate="${syndicateTag}"]`);
  ...
  // src/live/calendar-seasons.ts
  async function updateCalendarSeason() {
    const seasons = window.worldState?.KnownCalendarSeasons ?? [];
    if (seasons.length === 0) {
      setTimeout(() => {
    ...
    const now = Date.now();
    const activeSeason = seasons.find((s) => Number.parseInt(s.Activation.$date.$numberLong, 10) <= now && now < Number.parseInt(s.Expiry.$date.$numberLong, 10));
    if (activeSeason) {
  ...
  function applyCheckboxLinking(element, nowChecked) {
  ...
  function updateCircuitChoices() {
    ...
    document.querySelector("#circuit-frames").textContent = normalChoices.map((c) => choiceToDisplay(c)).join("\xA0\xB7 ") + " ";
    document.querySelector("#circuit-weapons").textContent = hardChoices.map((c) => choiceToDisplay(c)).join("\xA0\xB7 ") + " ";
    filterWeeklyMissions();
  }
  window.updateCircuitChoices = updateCircuitChoices;

  // src/live/completion-toggles.ts
  function setCompletionToggle(elm, completed) {
  ...
  function refreshAllCompletionToggles() {
  ...
  // src/helpers/mission-helpers.ts
  function canonicalizeMissionType(missionType) {
  ...
  var latestRenderedFissureTime = 0;
  var fissuresScheduledExpiries = /* @__PURE__ */ new Set();
  function buildFissureRow(dict, exports, renderedTierHeadings, fissure) {
    const node = exports.regions[fissure.Node];
    const tier = fissure.Modifier;
    const cardName = fissure.Category;
    const missionType = cardName === "rj-fissures" ? node.missionName.replace("/Lotus/Language/Missions/MissionName_", "") : node.missionType;
  ...
  function buildLocationCell(dict, node) {
    const td = document.createElement("td");
  ...
  async function updateFissures(forceRender = false) {
    ...
    const now = Date.now();
    let maxActivation = 0;
    for (const f of [...window.worldState.ActiveMissions, ...window.worldState.VoidStorms]) {
    ...
    const renderedTierHeadings = {
      fissures: /* @__PURE__ */ new Set(),
      "sp-fissures": /* @__PURE__ */ new Set(),
      "rj-fissures": /* @__PURE__ */ new Set()
    };
    const activeFissureExpiries = new Set(fissures.filter((f) => Date.now() >= Number.parseInt(f.Activation.$date.$numberLong, 10) && Date.now() < Number.parseInt(f.Expiry.$date.$numberLong, 10)).map((f) => Number.parseInt(f.Expiry.$date.$numberLong, 10)));
    for (const expiry of activeFissureExpiries) {
      ...
      }, Math.max(0, expiry + 6e4 - Date.now()));
    }
    const { fissureTiers } = globalThis;
    for (const fissure of fissures) {
      if (Date.now() < Number.parseInt(fissure.Activation.$date.$numberLong, 10)) {
        ...
        if (row) {
          tbody[fissure.Category].append(row);
        }
      }
    }
  ...
  function calculatePercentage(wsInvasion) {
    let progress = 1 - Math.abs(wsInvasion.Count / wsInvasion.Goal);
    progress = Math.max(0, progress);
    progress = Math.min(1, progress);
    return progress * 100;
  }
  function createInvasionProgressBar(wsInvasion, percentage) {
    ...
    const barPercentage = wsInvasion.Faction === "FC_INFESTATION" ? percentage : 100 * (wsInvasion.Count + wsInvasion.Goal) / (2 * wsInvasion.Goal);
    bar.style.width = `${barPercentage}%`;
    container.append(bar);
    return container;
  }
  function getDuplicateInvasionOids(invasions) {
    const nodeFirstOid = /* @__PURE__ */ new Map();
  ...
  function isPendingStart(invasion) {
    return Number.parseInt(invasion.Activation.$date.$numberLong, 10) > Date.now();
  }
  var INVASION_BUCKET_ACTIVE = 0;
  var INVASION_BUCKET_PENDING_START = 1;
  var INVASION_BUCKET_DUPLICATE = 2;
  function invasionBucket(invasion, isDuplicate) {
    if (isDuplicate) {
  ...
  function sortInvasions(invasions, duplicates, percentages) {
  ...
  function invasionRewardFilterKey(itemType) {
    const segment = itemType.replace(/^.+\//u, "");
    const isWeapon = /\/(?:Weapons|WeaponParts)\/[^/]+$/u.test(itemType);
    return isWeapon ? segment.replace(/SortieBlueprint$|[A-Z][a-z]+$/u, "") : segment.replace(/Blueprint$/u, "");
  }
  function isInvasionRewardShown(itemType) {
    return isFilterEnabled("invasions", `reward-${invasionRewardFilterKey(itemType)}`);
  }
  function formatActivationLabel(activationMs) {
    ...
    const totalMinutes = Math.floor((Date.now() - activationMs) / 6e4);
  ...
  function buildPercentageSpan(percentage, invasion, activationMs) {
    const span = document.createElement("span");
    ...
    const elapsedString = days >= 1 ? `${days}d${nbsp}${Math.floor((totalMinutes - days * 1440) / 60)}h${nbsp}${totalMinutes % 60}m` : hours >= 1 ? `${hours}h${nbsp}${totalMinutes % 60}m` : `${totalMinutes}m`;
  ...
  var pendingActivationTimeouts = /* @__PURE__ */ new Map();
  function buildPendingActivationBadge(activationMs, oid, td, invasion) {
  ...
  function buildPercentageCell(percentage, isDuplicate, nodeLabel, invasion) {
    const td = document.createElement("td");
  ...
  function buildInvasionHeading(invasion, node, nodeLabel, percentage) {
    const th = document.createElement("th");
    const tileset = node.missionType === "MT_ASSASSINATION" ? "GrineerAsteroidTileset" : getTileset(node);
    if (tileset) {
  ...
  async function buildInvasionRows(ctx) {
  ...
  async function updateInvasions() {
    if (!window.worldState?.Invasions) {
      return;
    }
    ...
    const activeInvasions = window.worldState.Invasions.filter((inv) => !inv.Completed);
    const duplicates = getDuplicateInvasionOids(activeInvasions);
    const percentages = new Map(activeInvasions.map((inv) => [inv._id.$oid, calculatePercentage(inv)]));
    ...
    for (const invasion of sorted) {
      ...
      const attackerItems = Array.isArray(invasion.AttackerReward) ? invasion.AttackerReward : invasion.AttackerReward.countedItems ?? [];
      const defenderItems = Array.isArray(invasion.DefenderReward) ? invasion.DefenderReward : invasion.DefenderReward.countedItems ?? [];
  ...
  var ALL_READ_TIMESTAMP_KEY = "live.news.all_read_timestamp";
  var READ_COLLECTION_KEY = "news_items_read";
  function generateNewsItemKey(item) {
    const linkPart = item.link || "";
    return `${linkPart}|${item.time}`;
  }
  function setNewsItemData(item, element) {
  ...
  function isNewsItemRead(item) {
    ...
    if (threshold !== void 0 && item.time <= threshold) {
      return true;
    }
    return getReadItems().includes(key);
  }
  function markNewsItemAsRead(item, element) {
  ...
  function markAllNewsAsRead() {
    ...
    const newThreshold = currentThreshold === void 0 ? newestTime : Math.max(currentThreshold, newestTime);
  ...
  // src/live/news.ts
  var latestRenderedNewsTime = 0;
  var newsNotifyAfter = 0;
  function updateNewsTicker(forceRender = false) {
    ...
    if (window.worldState) {
      ...
      for (const event of window.worldState.Events) {
        if (event.Date) {
          const time = Math.trunc(event.Date.$date.$numberLong / 1e3);
          if (time > highestTime) {
            highestTime = time;
          }
          let message = event.Messages?.find((x) => x.LanguageCode === languageCode)?.Message;
          message ??= event.Msg;
    ...
    for (let i = 0; i !== items.length; ++i) {
      ...
      }
      {
        ...
        span.dataset.activation = (items[i].time * 1e3).toString();
        span.textContent = window.formatActivation ? window.formatActivation(items[i].time * 1e3) : new Date(items[i].time * 1e3).toLocaleString();
        p.append(span);
      }
      {
        const span = document.createElement("span");
  ...
  // src/live/prune-stale-data.ts
  var PRUNE_DELAY_MS = 5e3;
  var PRUNE_INTERVAL_MS = MILLIS_PER_DAY;
  var lastPruneTime = 0;
  function pruneStaleOids() {
    ...
    const validMongoOids = /* @__PURE__ */ new Set();
    if (mongoCardsLoaded) {
      ...
      if (alertsBody) {
        for (const element of alertsBody.querySelectorAll("[data-oid]")) {
    ...
    const cleaned = allOids.filter((oid) => {
  ...
  function schedulePruneOnWorldState() {
  ...
  // src/live/sortie.ts
  function appendSortieLocation(td, node, tileset) {
  ...
  function initLiveSync() {
    ...
    globalThis.addEventListener("cloud-sync-pulled", () => {
      ...
      if (window.refreshCollapseStatus) {
        for (const elm of document.querySelectorAll("[data-collapse-toggle]")) {
          window.refreshCollapseStatus(elm);
        }
      }
      if (window.refreshNotifStatus) {
        for (const elm of document.querySelectorAll("[data-notif-toggle]")) {
      ...
      for (const checkbox of document.querySelectorAll("[data-filter-type]")) {
        const { filterType } = checkbox.dataset;
        if (filterType) {
  ...
  // src/faction-icons.ts
  var FACTION_ICON_PATHS = {
    ...
    FC_RED_VEIL: "/Lotus/Interface/Icons/StoreIcons/ShipDecos/Decorations/OrnamentRedVeil.png",
    // No proper faction icon exists; included for completeness
  ...
  function getFactionIconPath(faction) {
    ...
    return tfaction ? FACTION_ICON_PATHS[tfaction] : void 0;
  }
  ...
  var EQUIPMENT_CATEGORIES = {
    ...
    SpecialItems: { tooltip: "Special", icon: "", displayText: "\u{1F937}\u200D\u2640\uFE0F" }
  };
  var STALKER_ICON = "/Lotus/Interface/Icons/MarkedForDeathStalker.png";
  var ENEMY_FACTIONS = [
  ...
  function makeRenumber(tbody, previousObserver) {
    const renumber = () => {
      let rank = 1;
  ...
  function getEquipmentCategoryLabel(productCategory) {
    return EQUIPMENT_CATEGORIES[productCategory]?.tooltip;
  }
  function getEnemyFactionLabel(faction) {
  ...
  function initStatsFilterBar(filterBar, tbody, entries, presentKeys, onFilter) {
    filterBar.innerHTML = "";
  ...
  var enemyRankObserver;
  function augmentEnemyStats(profile) {
    ...
    const presentFactions = /* @__PURE__ */ new Set();
    let rowIndex = 0;
  ...
  var ARCHWING_PREFIX = "<ARCHWING> ";
  var ARCHWING_ICON = "/Lotus/Interface/Icons/StoreIcons/Gear/GenericArchwingSystems.png";
  var SHARED_DENOMINATOR_KEY = "Sentinels+Companions";
  var SHARED_DENOMINATOR_CATEGORIES = /* @__PURE__ */ new Set(["Sentinels", "KubrowPets", "MoaPets"]);
  function denominatorKeyFor(category) {
    return SHARED_DENOMINATOR_CATEGORIES.has(category) ? SHARED_DENOMINATOR_KEY : category;
  }
  var equipmentRankObserver;
  function computeCategoryTotals(weapons, exportWarframes, exportWeapons, exportSentinels) {
    const totals = {};
  ...
  function augmentArchwingName(nameCell) {
  ...
  async function augmentEquipmentStats(profile) {
    equipmentRankObserver?.disconnect();
    equipmentRankObserver = void 0;
    const equipmentFilterBar = document.querySelector("#equipment-filter-bar");
    const equipmentTbody = document.querySelector("#equipment-stats");
    ...
    const weapons = profile?.Stats?.Weapons ?? [];
    ...
    for (const item of weapons) {
      ...
      hoursCell.textContent = (equipTime / 3600).toLocaleString(void 0, { minimumFractionDigits: 1, maximumFractionDigits: 1 });
    }
  ...
  var MISSION_STATS = ["MissionsCompleted", "MissionsFailed", "MissionsQuit", "MissionsInterrupted", "MissionsDumped"];
  var CIPHER_STATS = ["CiphersSolved", "CiphersFailed"];
  function addTooltipDecoration(elm) {
    elm.style.cursor = "help";
    elm.style.textDecoration = "underline dotted";
    elm.dataset.bsToggle = "tooltip";
  }
  function addTimeStatTooltip(elm, stat, value) {
  ...
  function addStatPercentage(stat, value) {
    ...
    if (MISSION_STATS.includes(stat)) {
      ...
      if (total > 0) {
        const elm = document.querySelector(`#stat-${stat}`);
  ...
  function addCipherAvgTooltip(elm, profile) {
  ...
  function calcSyndicateRankPct(standing, rankMin, rankMax, level) {
    if (level < 0) {
  ...
  function appendSyndicateProgressBar(body, tag, standing, level, title) {
  ...
  var DEFAULT_BASE_URL = "https://warframe-api-front-proxy.dsinn69.workers.dev";
  async function rawRequest(path, includeAuth = false) {
    ...
    if (includeAuth) {
      ...
      const accessToken = getToken ? await getToken() : void 0;
      if (accessToken) {
  ...
  async function request(path) {
    ...
    return response.ok ? response.json() : void 0;
  }
  var WarframeApiFrontProxyClient = {
    ...
    async fetchProfile(platform, playerId) {
      ...
      if (!response.ok) {
        ...
        const nextFetchAvailableAt2 = retryAfter ? new Date(retryAfter).getTime() : void 0;
        return { status: response.status, data: null, nextFetchAvailableAt: nextFetchAvailableAt2 };
      }
      const json = await response.json();
      const nextFetchAvailableAt = json.nextFetchAvailableAt ? new Date(json.nextFetchAvailableAt).getTime() : void 0;
  ...
  var VALID_PLAYER_ID_REGEX = /^[\da-f]{24}$/u;
  var PLATFORM_STORAGE_KEY = "profile.platform";
  var ACCOUNT_ID_STORAGE_KEY = "profile.accountId";
  var PROFILE_DATA_STORAGE_KEY = "profile.data";
  var PROFILE_TIMESTAMP_STORAGE_KEY = "profile.dataFetchedAt";
  var NEXT_FETCH_AVAILABLE_AT_STORAGE_KEY = "profile.nextFetchAvailableAt";
  var platformSelect = document.querySelector("#platform-select");
  var currentAccountId = "";
  var cloudSyncEvent = waitForCloudSync();
  var initialProfilePromise = cloudSyncEvent.then(async () => {
  ...
  var profileLoadedManually = false;
  ...
  function showRateLimitNotice(expiryMs) {
    ...
    if (countdown) {
      countdown.replaceChildren(window.createShortTimerBadge(Math.floor(expiryMs / 1e3), "Pending Refresh"));
    }
  ...
  function updateProfileAge() {
    ...
    const absoluteTime = fetchDate.toLocaleString(void 0, {
      year: "numeric",
      month: "short",
      day: "numeric",
      hour: "2-digit",
  ...
  var invalidCharsMinTimeElapsed = false;
  var invalidCharsHideTimer;
