...
let viewMode = getDefaultViewMode();
let viewAnchorDate = getLocalToday();
...
function getIncursionsForDay(utcDayTimestamp) {
    if (epochDay === -1) {
        return undefined;
    }
...
function syncDropdownsToAnchor() {
    document.querySelector('#select-month').value = viewAnchorDate.getMonth().toString();
    document.querySelector('#select-year').value = viewAnchorDate.getFullYear().toString();
}
...
function makePlanetAbbrFull(node, nodeId) {
    const locationName = dict[node.name] ?? nodeId;
    const systemName = dict[node.systemName] ?? '';
    const formattedTileset = formatTileset(getTileset(node));
    const abbr = document.createElement('abbr');
...
function renderDayCell(cellDate, todayUtc) {
...
function renderTableRow(cellDate, todayUtc) {
    const utcTimestamp = localDateToUtcDayTimestamp(cellDate);
    const nodes = getIncursionsForDay(utcTimestamp);
    const isToday = utcTimestamp === todayUtc;
    const isPast = utcTimestamp < todayUtc;
    ...
    if (nodes) {
        for (const nodeId of nodes) {
            ...
            if (node) {
                const archwingIcon = makeArchwingIconIfApplicable(node);
                cell.append(makeFactionIcon(node), ' ', toTitleCase(dict[node.missionName] ?? node.missionName), ' @ ', makePlanetAbbr(node, nodeId), ...(archwingIcon ? [' ', archwingIcon] : []));
            }
            row.append(cell);
        }
    }
...
function renderCalendar(container, totalDays, viewStart, todayUtc) {
...
function renderTable(container, totalDays, viewStart, todayUtc) {
...
function renderList(container, totalDays, viewStart, todayUtc) {
...
export async function init() {
    ...
    const incursionsControls = document.querySelector('#incursions-controls');
    new ResizeObserver(() => {
    ...
    document.querySelector('#select-month').addEventListener('change', function () {
        viewAnchorDate.setMonth(Number.parseInt(this.value, 10));
        renderSchedule();
    });
    document.querySelector('#select-year').addEventListener('change', function () {
...
export async function loadData() {
    ...
    incursions = incursionText.split('\n')
        ...
        .map(array => [Number.parseInt(array[0], 10), array[1]]);
