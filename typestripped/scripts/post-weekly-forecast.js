...
const PROXY_BASE_URL = process.env.WARFRAME_API_FRONT_PROXY_BASE_URL
...
function findClanWeekly(entries) {
...
export async function formatArchimedea(worldState, archimedeaType, variantKeyPrefix, sectionTitle, find = findWeekly, showTimestamp = false) {
    const archimedeas = (worldState.Conquests ?? []).filter((c) => c.Type === archimedeaType);
    ...
    for (const mission of missions) {
        lines.push(`**${mission.type}**`, mission.variantDesc
...
export async function formatClanWeekly(worldState) {
    ...
    const regionName = resolveBonusRegion(entry.BonusRegion, dictEn);
...
//# sourceMappingURL=post-weekly-forecast.js.map