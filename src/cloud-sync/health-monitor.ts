...
type VisibilityFallbackOptions = {
	getCurrentUserId: () => string | undefined;
	getChannelState: () => string | undefined;
	isSyncing: () => boolean;
	getLastFreshTimestamp: () => number | undefined;
	pullFn: (userId: string) => Promise<void>;
	onPullComplete: (ts: number) => void;
...
export class CloudSyncHealthMonitor {
	...
	startHeartbeat(
		getChannelState: () => string | undefined,
		setLastFreshTimestamp: (ts: number) => void,
	): void {
	 ...
	 */
	startVisibilityFallback(options: VisibilityFallbackOptions): void {
		const {
			getCurrentUserId,
			getChannelState,
			isSyncing,
