...
async function setupPage(page: any) {
  ...
  await page.waitForSelector('#netracell-checks .completion-check', { timeout: 15000 });
  await page.waitForSelector('#labConquest-header .completion-check', { timeout: 15000 });
  await page.waitForSelector('#hexConquest-header .completion-check', { timeout: 15000 });
}
...
test('checking a middle Netracell checks all boxes to its left', async ({ page }) => {
  ...
  await expect(netracellChecks.nth(0).locator('i')).toHaveClass(/bi-check-square/);
  await expect(netracellChecks.nth(1).locator('i')).toHaveClass(/bi-check-square/);
  await expect(netracellChecks.nth(2).locator('i')).toHaveClass(/bi-check-square/);
  await expect(netracellChecks.nth(4).locator('i')).toHaveClass(/bi-square/);
});
...
test('unchecking a middle Netracell unchecks all boxes to its right', async ({ page }) => {
  ...
  await expect(netracellChecks.nth(0).locator('i')).toHaveClass(/bi-check-square/);
  await expect(netracellChecks.nth(2).locator('i')).toHaveClass(/bi-square/);
  await expect(netracellChecks.nth(3).locator('i')).toHaveClass(/bi-square/);
  await expect(netracellChecks.nth(4).locator('i')).toHaveClass(/bi-square/);
});
...
test("Netracell checkboxes are linked to the Archimedeas' and persist after reload", async ({ page }) => {
  ...
  const tempArch = page.locator('#hexConquest-header .completion-check');
  ...
  await tempArch.click();
  await expect(netracellChecks.nth(3).locator('i')).toHaveClass(/bi-check-square/);
  await expect(netracellChecks.nth(4).locator('i')).toHaveClass(/bi-check-square/);

  // All 5 Netracell boxes should now be checked
  for (let i = 0; i < 5; i++) {
    await expect(netracellChecks.nth(i).locator('i')).toHaveClass(/bi-check-square/);
  }
  ...
  await deepArch.click();
  for (let i = 0; i < 5; i++) {
    await expect(netracellChecks.nth(i).locator('i')).toHaveClass(/bi-square/);
  }
  ...
  for (let i = 0; i < 5; i++) {
    await expect(netracellChecksAfterReload.nth(i).locator('i')).toHaveClass(/bi-square/);
  }
});
