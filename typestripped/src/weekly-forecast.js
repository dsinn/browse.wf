...
export function mongoMs(d) {
    return Number.parseInt(d.$date.$numberLong, 10);
}
export function formatTabDate(ms) {
 ...
 */
export function buildTab(tabsElement, contentElement, id, label, activationMs, isActive, buildContent) {
    // Nav tab button
 ...
 */
export function getActiveTabActivation(tabsElement) {
    const active = tabsElement.querySelector('.nav-link.active');
 ...
 */
export function restoreActiveTab(tabsElement, activation) {
    const target = tabsElement.querySelector(`.nav-link[data-activation="${activation}"]`);
    if (!target) {
        return;
    }
    // Deactivate whichever tab buildTab marked active (index 0 / current)
    tabsElement.querySelector('.nav-link.active')?.classList.remove('active');
...
async function renderArchimedeaTabs(tabsElement, contentElement, archimedeas, archimedeaType, variantKeyPrefix, preserveActivation = undefined) {
    const now = Date.now();
...
async function renderDescentTabs(tabsElement, contentElement, descents, preserveActivation = undefined) {
    const now = Date.now();
    tabsElement.innerHTML = '';
    contentElement.innerHTML = '';
...
async function renderCalendarSeasonTabs(tabsElement, contentElement, seasons, preserveActivation = undefined) {
    const now = Date.now();
    tabsElement.innerHTML = '';
    contentElement.innerHTML = '';
    ...
    const columnsElement = document.querySelector('#calendar-season-columns');
    if (columnsElement) {
...
async function renderClanWeeklyColumns(columnsElement, entries) {
    columnsElement.innerHTML = '';
    if (entries.length === 0) {
        return;
    }
    ...
    for (const [i, entry] of entries.entries()) {
        ...
        const heading = document.createElement('h5');
        heading.className = 'mb-3';
        heading.textContent = formatTabDate(weekCountToActivationMs(entry.WeekCount));
        col.append(heading);
        col.append(panes[i]);
        row.append(col);
    }
 ...
 */
export function nextForecastPublishedMs() {
 ...
 */
export function initWeeklyMissionsNotice() {
...
export async function initWeeklyForecast(isRefresh = false) {
    const deepTabsElement = document.querySelector('#deep-archimedea-tabs');
    const temporalTabsElement = document.querySelector('#temporal-archimedea-tabs');
    const descentTabsElement = document.querySelector('#descendia-tabs');
    const calendarSeasonTabsElement = document.querySelector('#calendar-season-tabs');
    const clanWeeklyColumnsElement = document.querySelector('#clan-weekly-columns');
    const circuitForecastBodyElement = document.querySelector('#circuit-forecast-body');
    const teshinForecastHeaderElement = document.querySelector('#teshin-forecast-header');
    const teshinForecastBodyElement = document.querySelector('#teshin-forecast-body');
    ...
    const deepArchimedeas = (worldState.Conquests ?? []).filter((c) => c.Type === 'CT_LAB');
    if (deepTabsElement && deepArchimedeas.length > 0) {
        await renderArchimedeaTabs(deepTabsElement, document.querySelector('#deep-archimedea-content'), deepArchimedeas, 'CT_LAB', '/Lotus/Language/Conquest/MissionVariant_LabConquest_', deepActivation);
    }
    // Temporal Archimedea (CT_HEX)
    const temporalArchimedeas = (worldState.Conquests ?? []).filter((c) => c.Type === 'CT_HEX');
    if (temporalTabsElement && temporalArchimedeas.length > 0) {
        await renderArchimedeaTabs(temporalTabsElement, document.querySelector('#temporal-archimedea-content'), temporalArchimedeas, 'CT_HEX', '/Lotus/Language/Conquest/MissionVariant_HexConquest_', temporalActivation);
    }
    // Descendia
    const descents = worldState.Descents ?? [];
    if (descentTabsElement && descents.length > 0) {
        await renderDescentTabs(descentTabsElement, document.querySelector('#descendia-content'), descents, descentActivation);
    }
    // Calendar Seasons
    const calendarSeasons = worldState.KnownCalendarSeasons ?? [];
    if (calendarSeasonTabsElement && calendarSeasons.length > 0) {
        await renderCalendarSeasonTabs(calendarSeasonTabsElement, document.querySelector('#calendar-season-content'), calendarSeasons, calendarSeasonActivation);
    }
...
//# sourceMappingURL=weekly-forecast.js.map